...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.administrator?.length > 0) {
      const create_administrator = body.administrator;
      body.administrator = {
        create: create_administrator,
      };
    } else {
      delete body.administrator;
    }
    if (body?.holding?.length > 0) {
      const create_holding = body.holding;
      body.holding = {
        create: create_holding,
      };
    } else {
      delete body.holding;
    }

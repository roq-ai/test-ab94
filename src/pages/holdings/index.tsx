...
import { getHoldings, deleteHoldingById } from 'apiSdk/holdings';
import { HoldingInterface } from 'interfaces/holding';

type ColumnType = ColumnDef<HoldingInterface, unknown>;
...
export function HoldingListPage(props: HoldingListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getHoldings({
        relations: ['organization', 'stock', 'transaction.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HoldingInterface>>(
    () => `/holdings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: HoldingInterface) => {
    if (hasAccess('holding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/holdings/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'purchase_price', header: 'purchase_price', accessorKey: 'purchase_price' },
    {
      id: 'purchase_date',
    ...
    hasAccess('stock', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'stock',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/stocks/view/${record.stock?.id}`}>
              {record.stock?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/holdings/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('holding', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/holdings/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('holding', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="holding" />
        </Flex>

        {hasAccess('holding', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/holdings/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

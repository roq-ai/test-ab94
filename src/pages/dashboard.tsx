...
function DashboardPage() {
  ...
  const administratorStat = useSWR<PaginatedInterface<AdministratorInterface>>(
    () => `/administrators`,
    () => getAdministrators({ limit: 1, offset: 0 }),
  );

  const holdingStat = useSWR<PaginatedInterface<HoldingInterface>>(
    () => `/holdings`,
    () => getHoldings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: administratorStat?.data?.totalCount,
      isLoading: administratorStat.isLoading,
    },
    {
      title: 'Holdings',
      result: holdingStat?.data?.totalCount,
      isLoading: holdingStat.isLoading,
    },
  ];

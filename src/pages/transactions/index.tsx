  ...
  const columns: ColumnType[] = [
    { id: 'transaction_type', header: 'Transaction Type', accessorKey: 'transaction_type' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.transaction_date ? format(parseISO(record?.transaction_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'transaction_price', header: 'transaction_price', accessorKey: 'transaction_price' },
    ...
    hasAccess('holding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'holding',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/holdings/view/${record.holding?.id}`}>
              {record.holding?.purchase_price}
            </Link>
          ),
        }
      : null,

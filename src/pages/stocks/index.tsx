...
type ColumnType = ColumnDef<StockInterface, unknown>;
...
export function StockListPage(props: StockListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getStocks({
        relations: [, 'holding.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'ticker_symbol.contains', 'market.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StockInterface>>(
    () => `/stocks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StockInterface) => {
    if (hasAccess('stock', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/stocks/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'name', header: 'Name', accessorKey: 'name' },
    { id: 'ticker_symbol', header: 'Ticker Symbol', accessorKey: 'ticker_symbol' },
    { id: 'market', header: 'Market', accessorKey: 'market' },
    { id: 'price', header: 'price', accessorKey: 'price' },
    { id: 'quantity', header: 'quantity', accessorKey: 'quantity' },
    hasAccess('holding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'holding',
          header: 'Holding',
          accessorKey: 'holding',
          cell: ({ row: { original: record } }: any) => record?._count?.holding,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/stocks/view/${record.id}`} passHref legacyBehavior>
                <Button
